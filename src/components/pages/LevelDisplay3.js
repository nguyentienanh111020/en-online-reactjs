...
function LevelDisplay3() {
  ...
  useEffect(() => {
    const docRef = doc(db, "advanced", id);
    ...
    const getQuest = async () => {
      const questRef = collection(db, "/advanced/" + id + "/question");
    ...
    const getQuest2 = async () => {
      const questRef2 = collection(db, "/advanced/" + id + "/question 2");
    ...
    const getQuest3 = async () => {
      const questRef3 = collection(db, "/advanced/" + id + "/question 3");
    ...
    const getQuest4 = async () => {
      const questRef4 = collection(db, "/advanced/" + id + "/question 4");
      const data = await getDocs(questRef4);
      setQuest4(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    };
    getQuest4();

    const getQuest5 = async () => {
      const questRef5 = collection(db, "/advanced/" + id + "/question 5");
      const data = await getDocs(questRef5);
      setQuest5(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    };
    getQuest5();
  }, []);
          ...
          <Accordion defaultActiveKey={["0"]} alwaysOpen>
            ...
            <Accordion.Item eventKey="2">
              ...
              <Accordion.Body>
                ...
                <hr />
                {level.title5}
                {quest5.map((quest5, idx) => {
                  if (quest5.isCorrect == "true") {
                    return (
                      <div key={idx}>
                        <input
                          type="radio"
                          ...
                          id="true5"
                        />
                        <label className="isCorrect">{quest5.text}</label>
                      </div>
                    );
                  } else {
                    return (
                      <div key={idx}>
                        <input
                          ...
                          value="Yes"
                        />
                        <label className="inCorrect">{quest5.text}</label>
                      </div>
                    );
                  }
                })}
                  ...
                  onClick={function () {
                    ...
                    if (result == 5) {
                      ...
                      updateDoc(doc(db, "users", user.uid), {
                        advanced: arrayUnion(`${level.name}`),
                      });
                    } else {
                      document.getElementById(
                        "result"
