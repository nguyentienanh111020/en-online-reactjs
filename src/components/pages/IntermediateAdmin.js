...
function IntermediateAdmin() {
  ...
  const handleAdd = async (e) => {
    ...
    await setDoc(doc(db, "intermediate", "no" + String(response)), {
      ...data,
    ...
    for (let i = 0; i < 3; i++) {
      await setDoc(
        doc(db, "intermediate", "no" + String(response), "question", String(i)),
        {
  ...
  const handleUpdate = async (e) => {
    ...
    await updateDoc(doc(db, "intermediate", String(id)), {
      ...update,
    });
    ...
    for (let i = 0; i < 3; i++) {
      if (document.getElementById(`textU${i}`).value != "") {
        await setDoc(
          doc(db, "intermediate", String(id), "question", String(i)),
          {
    ...
    for (let i = 0; i < 3; i++) {
      if (document.getElementById(`textUq2${i}`).value != "") {
        await setDoc(
          doc(db, "intermediate", String(id), "question 2", String(i)),
          {
    ...
    for (let i = 0; i < 3; i++) {
      if (document.getElementById(`textUq3${i}`).value != "") {
        await setDoc(
          doc(db, "intermediate", String(id), "question 3", String(i)),
          {
    ...
    for (let i = 0; i < 3; i++) {
      if (document.getElementById(`textUq4${i}`).value != "") {
        await setDoc(
          doc(db, "intermediate", String(id), "question 4", String(i)),
          {
  ...
  const handleDelete = async (e) => {
    ...
    for (let i = 0; i < 3; i++) {
      await deleteDoc(
        doc(db, "intermediate", String(deleteId), "question 2", `${i}`)
      );
    }

    for (let i = 0; i < 3; i++) {
      await deleteDoc(
        doc(db, "intermediate", String(deleteId), "question", `${i}`)
      );
    }
    for (let i = 0; i < 3; i++) {
      await deleteDoc(
        doc(db, "intermediate", String(deleteId), "question 3", `${i}`)
      );
    }
    for (let i = 0; i < 3; i++) {
      await deleteDoc(
        doc(db, "intermediate", String(deleteId), "question 4", `${i}`)
      );
    }

    await deleteDoc(doc(db, "intermediate", String(deleteId)));
  ...
  return (
    <div className="container" style={{ paddingBottom: "120px" }}>
      <Accordion>
        <h1 id="admin-title">Intermediate</h1>
          ...
          <Accordion.Body>
            <Form onSubmit={handleAdd}>
              ...
              <Form.Group>
                ...
                <Form.Control
                  type="input"
                  placeholder="Answer 3 for Question 3"
                  id="textq32"
                  required
                />
                <Form.Select
...
export default IntermediateAdmin;
